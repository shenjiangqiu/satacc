...
TEST_CASE("cache_interface", "[basic][core][componet]") {
  ...
  auto m_c = cache_interface(16, 1 << 16, 16, 16, 8, "error", current_cycle);

  m_c.current_cycle = 0;
  SECTION("first test") {
    m_c.in_request_queues[0].push_back(std::make_unique<cache_interface_req>(
    ...
    m_c.in_request_queues[0].push_back(std::make_unique<cache_interface_req>(
        AccessType::ReadWatcherData, 0, 0, 0, new_wrap1));
    std::cout << "current cycle:" << current_cycle << std::endl;
    while (m_c.out_resp_queues[0].empty()) {
      m_c.cycle();
      current_cycle++;
    ...
    m_c.in_request_queues[0].push_back(std::make_unique<cache_interface_req>(
        AccessType::ReadWatcherData, 16, 0, 0, new_wrap1));
    std::cout << "current cycle:" << current_cycle << std::endl;
    while (m_c.out_resp_queues[0].empty()) {
      m_c.cycle();
      current_cycle++;
