...
private:
  ...
  unsigned long long num_inflight_request = 0; // inside unit
  double average_inflight = 0;
  std::deque<req_ptr>
      waiting_value_watcher_queue; // int : remaining value to be processed
  std::deque<req_ptr>
      waiting_read_watcher_queue; // int: remaing watcher to be read
  std::deque<req_ptr>
      waiting_process_queue; // int : remaining watcher to be processed
...
public:
  ...
  int next_c(int total) { return next_clause = (next_clause + 1) % total; }

  std::string get_line_trace() const override {
    return fmt::format(
        "{}:{} total_idle:{} total_busy:{} total_waiting_data:{} "
        "total_waiting_value:{} total_waiting_task:{} \naverage_inflight_watcher {}",
        "watcher", std::to_string(get_busy_percent()), total_idle, total_busy,
        total_idle_no_watcher_data, total_idle_no_value, total_idle_no_task,average_inflight);
  }
  std::string get_internal_size() const override {
    auto r = fmt::format(
        "name w_meta w_v w_d w_p in_ in_m out_m out_s out_write\n {}-{} {} {} "
        "{} {} {} {} {} {} {}",
        "watcher", m_id, waiting_read_meta_queue.size(),
        waiting_value_watcher_queue.size(), waiting_read_watcher_queue.size(),
        waiting_process_queue.size(), in_task_queue.size(),
        in_memory_resp_queue.size(), out_memory_read_queue.size(),
        out_send_queue.size(), out_write_watcher_list_queue.size());
    return r;
  }
  bool empty() const override {
    bool e = waiting_read_meta_queue.empty() and
             waiting_value_watcher_queue.empty() and
             waiting_read_watcher_queue.empty() and
             waiting_process_queue.empty() and in_task_queue.empty() and
             in_memory_resp_queue.empty() and out_memory_read_queue.empty() and
             out_send_queue.empty() and out_write_watcher_list_queue.empty();
    return e;
  }
