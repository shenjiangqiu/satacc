...
private_cache::private_cache(int asso, int total_size, uint64_t &tcurrent_cycle)
    : componet(tcurrent_cycle), m_cache(asso, total_size) {}
...
bool private_cache::from_in_to_out() {
  ...
  if (!in_request.empty() and out_send_q.size() < out_size) {
    ...
    case AccessType::ReadWatcherValue:
      ...
      addr = as->get_blocker_addr(watcherId);

      break;
    case AccessType::ReadClauseValue:
      addr = as->get_clause_literals_addr(watcherId)[clauseId];
      // cache_type = 1;
      break;
    default:
      throw;
