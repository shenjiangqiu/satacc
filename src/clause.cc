...
bool clause::do_cycle() {
  ...
  if (!busy) {
    ...
    if (on_going_requests > 0) {
      stall_on_going_request++;
    } else if (in_task_queue.empty()) {
      stall_no_task++;
    } else {
      stall_other++;
    }
...
bool clause::task_to_data_waiting() // get the task and send to data waiting
                                    // queue
{
  bool busy = false;
...
bool clause::data_waiting_to_mem_out() // get from data_waiting queue, and get
                                       // clause detail
{
  bool busy = false;
  if (!clause_data_read_waiting_queue.empty()) {
    ...
    auto total_clause_size = clause_data_read_waiting_queue.front()
                                 ->as->get_clause_literal(index)
                                 .size();
    // error FIXME
    // should copy here,not move
    // auto copy = std::unique_ptr<cache_interface_req>(new
    // cache_interface_req(*clause_data_read_waiting_queue.front()));
...
bool clause::value_waiting_to_mem_out() // get from value_waiting, and get value
                                        // details
{
  bool busy = false;
