...
cache_interface::cache_interface(unsigned int total_size,
                                 unsigned num_partition, bool ideal_dram,
                                 bool ideal_l3, unsigned num_ports,
                                 std::string config_name, uint64_t &t)
    : cache_interface(16, total_size >> 10, 192, 4, num_partition, ideal_dram,
...
uint64_t cache_interface::from_cache_addr_to_real_addr(uint64_t cache_addr,
...
bool cache_interface::from_in_to_cache() {
  ...
  for (auto &&[i, in_request_queue] : enumerate(in_request_queues)) {
    ...
    while (!in_request_queue.empty() and miss_queue.size() < miss_size and
           remain_ports-- > 0) {
      busy = true;
      this_busy[i] = true;
      ...
      if (ideal_l3) {
        if (req->type == AccessType::WriteClause or
            req->type == AccessType::WriteWatcherList or
      ...
      cache_result =
          m_caches[i].access(cache_addr, cache_type,
                             (int)req->type); // fix the cache access behavior,
                                              // now can support write access

      // here we need to deal with the write traffic,
      if (cache_type == sjq::cache::write) {
        if (cache_result == sjq::cache::miss and
            m_caches[i].get_last_evict().get_tag() != 0 and
            m_caches[i].get_last_evict().isDirty()) {
          miss_queue.emplace_back(
              false, from_cache_addr_to_real_addr(
                         m_caches[i].get_last_evict().get_tag() << 6, i));
          cache_interface_req n_req;
          ...
          if (t_type == AccessType::ReadClauseData) {
            ...
            assert(t_type == AccessType::WriteWatcherList or
                   t_type == AccessType::WriteClause);
          }
          n_req.type = t_type;
          // std::cout << t_type << std::endl;
      ...
      case sjq::cache::hit:
        ...
        delay_resp_queues[i].push_back(
            std::make_pair(internal_cycle, std::move(req)));
        /* code */
        break;
      ...
      case sjq::cache::miss: {
        ...
        if (evict_entry.get_tag() != 0 and evict_entry.isDirty()) {
          miss_queue.push_back({false, from_cache_addr_to_real_addr(
                                           evict_entry.get_tag() << 6, i)});
          // std::cout << "send write:" << addr << std::endl;
          cache_interface_req n_req;
          auto t_type = (AccessType)m_caches[i].get_last_evict().getMType();
          // std::cout << t_type << std::endl;
        ...
        // std::cout << fmt::format("read from {}: {}", i, block_addr) <<
        // std::endl;
...
bool cache_interface::from_dramresp_to_resp() {
  ...
  if (!dram_resp_queue.empty()) {
    ...
    auto addr =
        dram_resp_queue.front(); // this will be the blockaddr we send to dram
    ...
    if (delay_resp_queue.size() < delay_q_size) {
      ...
      for (auto &i : v) {
        delay_resp_queue.push_back(
            std::make_pair(internal_cycle, std::move(i)));
      }
...
std::string cache_interface::get_line_trace() const {
  ...
  ret +=
      ...
      "\nReadWatcherData,ReadWatcherValue,ReadWatcherMetaData,ReadClauseData,"
      "ReadClauseValue,WriteWatcherList,WriteClause,EvictWrite,WriteMissRead " +
  ...
  for (auto i = 0u; i < n_partition; i++) {
    ...
    ret +=
        (fmt::format("\n{}:{} {} {}\n", "cache_interface", busys[i], idles[i],
         ...
         fmt::format("c.num_hit {} ,c.num_hit_reserved {}  ,c.num_miss {} "
                     ",c.num_res_fail {} \n",
                     c.num_hit, c.num_hit_reserved, c.num_miss,
                     c.num_res_fail) +
         fmt::format("the_hist {}\n", access_hists[i]));
  }
  ...
  ret += fmt::format(
      ...
      m_stats.read_watcher_data_hit, m_stats.read_watcher_value_hit,
      m_stats.read_clause_data_hit, m_stats.read_clause_value_hit,
      m_stats.write_watcher_list_hit, m_stats.write_clause_hit,
...
std::string cache_interface::get_internal_size() const {
  ...
  for (auto i = 0u; i < n_partition; i++) {
    ret += fmt::format("name delay in out \n {} {} {} {}\n",
                       "cache_interface:", delay_resp_queues[i].size(),
                       in_request_queues[i].size(), out_resp_queues[i].size());
  }

  ret += fmt::format("name addr_to_req missq dram_r  \n {} {} {} {}\n",
                     "cache_interface:", addr_to_req.size(), miss_queue.size(),
...
bool cache_interface::empty() const {
  bool empty = true;
  for (auto i = 0u; i < n_partition; i++) {
    empty = empty and delay_resp_queues[i].empty() and
            in_request_queues[i].empty() and out_resp_queues[i].empty();
  }
  return empty and addr_to_req.empty() and miss_queue.empty() and
         dram_resp_queue.empty() and m_mem.empty();
}
...
cache_interface::cache_interface(int cache_set_assositive, int cache_num_sets,
                                 int cache_mshr_entries,
                                 int cache_mshr_maxmerge,
                                 unsigned num_partition, bool tis_ideal_memory,
                                 bool ideal_l3, unsigned num_ports,
                                 std::string config_name, uint64_t &t)
    : componet(t),
      m_caches(num_partition,
               sjq::cache(cache_set_assositive, cache_num_sets / num_partition,
                          sjq::cache::lru, cache_mshr_entries,
                          cache_mshr_maxmerge, "l3cache")),
      m_mem(config_name, 64, t), n_partition(num_partition),
      this_busy(num_partition, false), busys(num_partition, 0),
      idles(num_partition, 0), delay_resp_queues(num_partition),
      access_hists(n_partition, {0}), is_ideal_dram(tis_ideal_memory),
      ideal_l3(ideal_l3), num_ports(num_ports),
      in_request_queues(num_partition), out_resp_queues(num_partition) {

  // m_mem("DDR4_4Gb_x16_2133_2.ini", "./", std::bind(read_call_back,
  // std::placeholders::_1), std::bind(write_call_back, std::placeholders::_1));
}
